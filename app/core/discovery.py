...
from app.core.device import Topology, Link
    ...
    async def _discover_device(self, ip: str, username: str, password: str, port: int, depth: int) -> None:
        ...
        async with self.semaphore:
            try:
                ...
                log.error(f"发现设备 {ip} 失败", error=str(e))
...
""")
    ...
    if result.success:
        ...
        if choice == "1":
            log.info(result.export_topology("json"))
        elif choice == "2":
            log.info(result.export_topology("mermaid"))
        elif choice == "3":
            log.info(result.export_topology("graphviz"))
    else:
        log.info(f"\n[FAIL] {result.message}")
